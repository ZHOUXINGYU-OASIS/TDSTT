...
def STT3_pred(STT3, x, y, z):
    """
    Calculate the third-order term using STT
    """
    lx = len(x)
    lpred = len(STT3)
    pred = np.zeros([lpred])
    for i in range(lpred):
        for k1 in range(lx):
            for k2 in range(lx):
                for k3 in range(lx):
                    pred[i] = pred[i] + 1 / 6 * STT3[i, k1, k2, k3] * x[k1] * y[k2] * z[k2]
    return pred
...
def STT2_Combine(N1, N2, STM, STT):
    """
    Combine two STTs
    """
    len_i = len(N1[:,0])
...
def DSTT2_Pred_mu_P(P0, STM, DSTT, R, dim):
